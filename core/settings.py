...
CONDENSE_ON_INFO_KEYWORDS = (
"attacker", "reputation", "scanner", "user agent", "tor exit", "port scanning", "potential infection")
CONDENSED_EVENTS_FLUSH_PERIOD = 10
LOW_PRIORITY_INFO_KEYWORDS = (
"reputation", "attacker", "spammer", "abuser", "malicious", "dnspod", "nicru", "crawler", "compromised", "bad history")
...
SUSPICIOUS_CONTENT_TYPES = ("application/vnd.ms-htmlhelp", "application/x-bsh", "application/x-chm", "application/x-sh",
...
SUSPICIOUS_HTTP_REQUEST_REGEXES = (
    ...
    ("potential xss injection",
     r"<script.*?>|\balert\(|(alert|confirm|prompt)\((\d+|document\.|response\.write\(|[^\w]*XSS)|on(mouseover|error|focus)=[^&;\n]+\("),
    ("potential xxe injection", r"\[<!ENTITY"),
    ("potential data leakage",
     r"im[es]i=\d{15}|(mac|sid)=([0-9a-f]{2}:){5}[0-9a-f]{2}|sim=\d{20}|([a-z0-9_.+-]+@[a-z0-9-.]+\.[a-z]+\b.{0,100}){4}|(telnum|telcompany)=[a-zA-Z0-9-]+"),
    ("config file access", r"\.ht(access|passwd)|\bwp-config\.php"),
    ("potential remote code execution",
     r"\$_(REQUEST|GET|POST)\[|xp_cmdshell|shell_exec|\bping(\.exe)? -[nc] \d+|timeout(\.exe)? /T|tftp -|wget http|curl -O|sh /tmp/|cmd\.exe|/bin/(ba)?sh\b|2>&1|\b(cat|ls) /|chmod [0-7]{3,4}\b|chmod +x\b|nc -l -p \d+|>\s*/dev/null|-d (allow_url_include|safe_mode|auto_prepend_file)|\$\{IFS\}"),
    ("potential directory traversal",
     r"(\.{2,}[/\\]+){3,}|/etc/(passwd|shadow|issue|hostname)|[/\\](boot|system|win)\.ini|[/\\]system32\b|%SYSTEMROOT%"),
    ("potential web scan",
...
"fql", "yql", "ads", "../images/", "../themes/", "../design/", "../scripts/", "../assets/", "../core/", "../js/",
"/gwx/")
...
IGNORE_EVENTS = set()
STATIC_IPCAT_LOOKUPS = {"shadowserver.org": (
"184.105.139.66-184.105.139.126", "184.105.247.194-184.105.247.254", "74.82.47.1-74.82.47.63",
"216.218.206.66-216.218.206.126"), "labs.rapid7.com": ("71.6.216.32-71.6.216.63",), "shodan.io": (
"66.240.192.138", "66.240.236.119", "71.6.135.131", "71.6.165.200", "71.6.167.142", "82.221.105.6", "82.221.105.7",
"85.25.43.94", "85.25.103.50", "93.120.27.62", "104.131.0.69", "104.236.198.48", "162.159.244.38", "188.138.9.50",
"198.20.69.74", "198.20.69.98", "198.20.70.114", "198.20.87.98", "198.20.99.130", "208.180.20.97", "209.126.110.38"),
                        "eecs.umich.edu": ("141.212.121.0-141.212.121.255", "141.212.122.0-141.212.122.255"),
                        "netsec.colostate.edu": (
                        "129.82.138.12", "129.82.138.31", "129.82.138.32", "129.82.138.33", "129.82.138.34",
                        "129.82.138.44"), "ant.isi.edu": ("128.9.168.98", "203.178.148.18", "203.178.148.19"),
                        "eecs.berkeley.edu": (
                        "169.229.3.89", "169.229.3.90", "169.229.3.91", "169.229.3.92", "169.229.3.93", "169.229.3.94"),
                        "openresolverproject.org": ("204.42.253.2", "204.42.254.5"),
                        "opensnmpproject.org": ("204.42.253.130",), "openntpproject.org": ("204.42.253.131",),
...
DLT_OFFSETS = {0: 4, 1: 14, 6: 22, 7: 6, 8: 16, 9: 4, 10: 21, 117: 48, 18: 4,
               12 if sys.platform.find('openbsd') != -1 else 108: 4,
               14 if sys.platform.find('openbsd') != -1 else 12: 0, 113: 16}
...
maltrail_config = AttribDict({"TRAILS_FILE": DEFAULT_TRAILS_FILE})
trails = TrailsDict()
...
def _get_total_physmem():
    ...
    if not retval:
        try:
            retval = int(re.search(r"hw\.(physmem|memsize):\s*(\d+)",
                                   subprocess.check_output("sysctl hw", shell=True, stderr=subprocess.STDOUT)).group(2))
        except:
            pass

    if not retval:
        try:
            retval = 1024 * int(re.search(r"\s+(\d+) K total memory", subprocess.check_output("vmstat -s", shell=True,
    ...
    if not retval:
        try:
            retval = int(re.search(r"Mem:\s+(\d+)",
                                   subprocess.check_output("free -b", shell=True, stderr=subprocess.STDOUT)).group(1))
        except:
            pass

    if not retval:
        try:
            retval = 1024 * int(re.search(r"KiB Mem:\s*\x1b[^\s]+\s*(\d+)",
                                          subprocess.check_output("top -n 1", shell=True,
...
def read_config(config_file):
            ...
            if array and line.startswith(' '):
                ...
                if array == "IP_ALIASES" and any(_ in line.split(':')[0] for _ in ('/', '-')):
                    for addr in expand_range(line.split(':')[0]):
                        maltrail_config[array].append("%s:%s" % (addr, line.split(':', 1)[-1]))
                else:
                    maltrail_config[array].append(line)
    ...
    if maltrail_config.SSL_PEM:
        maltrail_config.SSL_PEM = maltrail_config.SSL_PEM.replace('/', os.sep)

    if maltrail_config.USER_WHITELIST:
        if ',' in maltrail_config.USER_WHITELIST:
            print(
                "[x] configuration value 'USER_WHITELIST' has been changed. Please use it to set location of whitelist file")
        elif not os.path.isfile(maltrail_config.USER_WHITELIST):
            exit("[!] missing 'USER_WHITELIST' file '%s'" % maltrail_config.USER_WHITELIST)
        else:
            read_whitelist()

    if maltrail_config.USER_IGNORELIST:
        if not os.path.isfile(maltrail_config.USER_IGNORELIST):
    ...
    if not maltrail_config.TRAILS_FILE:
        ...
        maltrail_config.TRAILS_FILE = os.path.abspath(os.path.expanduser(maltrail_config.TRAILS_FILE))

    if int(os.environ.get("MALTRAIL_DREI", 0)) > 0:
        maltrail_config.SHOW_DEBUG = True
...
def read_whitelist():
    ...
    if maltrail_config.USER_WHITELIST and os.path.isfile(maltrail_config.USER_WHITELIST):
        with open(maltrail_config.USER_WHITELIST, "r") as f:
            for line in f:
                line = line.strip()
                if not line or line.startswith('#'):
                    continue
...
def read_ignorelist():
    ...
    if maltrail_config.USER_IGNORELIST and os.path.isfile(maltrail_config.USER_IGNORELIST):
        add_ignorelist(maltrail_config.USER_IGNORELIST)
